...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      response = await fetch(`${BACKEND_URL}/api/v1/inquiries`, {
        method: "POST",
      ...
      // If localhost failed due to IPv6/IPv4 mismatch, try 127.0.0.1 fallback
      if (BACKEND_URL.includes("localhost")) {
        const fallbackUrl = BACKEND_URL.replace("localhost", "127.0.0.1");
        response = await fetch(`${fallbackUrl}/api/v1/inquiries`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
          signal: AbortSignal.timeout(10000),
    ...
    console.error("Inquiries API proxy error:", error);
